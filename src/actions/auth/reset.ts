...
import { providerMap } from "@/actions/auth/auth-providers";

// Type guard to check if error is a FirebaseAuthError
function isFirebaseAuthError(
  error: any
): error is { errorInfo: { code: string; message: string } } {
...
export const resetPassword = async (values: z.infer<typeof ResetSchema>) => {
  ...
  try {
    ...
    const externalProviders = Object.values(providerMap).map(
      (provider) => provider.PROVIDER_ID
    );

    const isExternalProvider = user.providerData.some((provider) => {
      return (externalProviders as readonly string[]).includes(
        provider.providerId
      );
    });
    ...
    if (isExternalProvider) {
      return {
        error:
          "Cannot reset password for accounts created with a third party service.",
      };
    }
